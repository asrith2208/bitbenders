    ...
    <Link to={`/projects/${project.id}`} className="block h-full group">
      <div className="relative">
        ...
        <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-all duration-300 flex items-center justify-center">
          <span className="material-icons text-white text-6xl opacity-70 group-hover:opacity-100 group-hover:scale-110 transition-transform duration-300">
            play_circle
          </span>
        </div>
      </div>
      <div className="p-6">
        ...
        <div className="text-right text-sm text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700 pt-2">
          {project.level}
        </div>
      </div>
    </Link>
...
const ProjectsPage: React.FC = () => {
  ...
  const handleFilterChange = (
    filterType: 'domain' | 'language' | 'level',
    value: string
  ) => {
    setFilters((prev) => ({
      ...prev,
  ...
  return (
    <main className="container mx-auto px-6 py-20">
      <ScrollReveal>
        ...
        <p className="text-lg text-gray-500 dark:text-gray-400 mt-2 text-center mb-12">
          Filter, find, and start building your next masterpiece.
        </p>
      </ScrollReveal>
      ...
      <ScrollReveal delay={100}>
        <div className="mb-12 space-y-6">
          {Object.keys(PROJECT_FILTERS).map((filterKey) => (
            <div
              key={filterKey}
              className="flex flex-wrap items-center justify-center gap-2 md:gap-4 p-4 rounded-xl bg-gray-50 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800"
            >
              <span className="font-bold text-sm uppercase mr-4 text-gray-600 dark:text-gray-400">
                {filterKey}:
              </span>
              {PROJECT_FILTERS[filterKey as keyof typeof PROJECT_FILTERS].map(
                (option) => (
                  <FilterButton
                    key={option}
                    label={option}
                    onClick={() =>
                      handleFilterChange(
                        filterKey as 'domain' | 'language' | 'level',
                        option
                      )
                    }
                    isActive={
                      filters[filterKey as 'domain' | 'language' | 'level'] ===
                      option
                    }
                  />
                )
